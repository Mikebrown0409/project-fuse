...
fn test_selective_disclosure_with_real_manifest() {
    ...
    match fuse_core::zkvm::generate_proof(spec_json, &system_data_json, ProverType::Local) {
        Ok((receipt_bytes, journal_output, journal_bytes)) => {
            let result = journal_output.result;
            ...
            assert_eq!(result, verified_journal_output.result, "Verified result should match");
...
fn test_c2pa_end_to_end_workflow() {
    ...
    match fuse_core::zkvm::generate_proof(spec_json, &system_data_json, ProverType::Local) {
        Ok((receipt_bytes, journal_output, journal_bytes)) => {
            ...
            let (verified_journal_output, verified_journal) = verify_result.unwrap();
            assert_eq!(result, verified_journal_output.result);
